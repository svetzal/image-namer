...
### Changed
- Updated dependency lockfile to latest compatible versions (filelock 4.0.1 -> 4.0.3, flake8 7.4.0 -> 7.4.1, httpcore2 2.13.0 -> 2.13.1, httpx2 2.13.0 -> 2.13.1, openai 3.19.0 -> 3.19.2, pytz 2026.3.post1 -> 2026.4). `pydantic-core` remains at 2.46.5 because pydantic 2.13.5 pins it exactly. No known vulnerabilities found via `pip-audit`; all quality gates (pytest, flake8, mypy) pass unchanged.
...
### Changed
- Bump patch version to 2.0.3 for new release (no functional code changes)

## [2.0.2] - 2025-11-03

### Fixed
- Added `constants` module to `py-modules` in `pyproject.toml` to fix `ModuleNotFoundError` when installing via pipx
- GUI (`image-namer-ui`) now works correctly after pipx installation

## [2.0.1] - 2025-11-02

### Changed
- Added GUI screenshot to README.md for better visibility on GitHub and documentation site
